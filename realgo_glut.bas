...
Dim Shared As String inputstring
Dim Shared As Integer mousex, mousey, wantinput, oldmx, oldmy, rbuttoncnt, lbuttoncnt
Dim Shared As Integer hovering, showtex, showar
...
Sub mytextout (x As Double, y As Double, z As Double, s As String)
	...
	For t = 1 To Len(s)
		...
		#define GLUT_BITMAP_9_BY_15 Cast (Any Ptr, 2)
		#EndIf
		glutBitmapCharacter (GLUT_BITMAP_9_BY_15, Asc(Mid(s,t,1)))
	Next
End Sub
...
Sub showtexture ()
	...
	glTexImage2D (GL_TEXTURE_2D, 0, GL_RGBA8, BAS, BAS, 0, GL_RGBA, GL_UNSIGNED_BYTE, texdat)
...
Sub doRender Cdecl ()
	...
	ElseIf wantinput>=0 Then

		viewport (VMENU)
		drawmenu ()
		If wantinput=1 Then
			glDisable (GL_DEPTH_TEST)
			mybox (30, 17*16+4, 280, 15*16+4, 0.1, CBOARD)
			mycolor (stonecolor (stonenr))
			mytextout (38, 16*16, 0.1, "input x,y: "+inputstring)
...
Sub doKeyboard Cdecl (kbcode As UByte, mx As Integer, my As Integer)
	Dim As String i, s
	Dim As Integer t, x, y, nx, ny, dx, dy
	...
	If wantinput=-2 Then
		...
		If gametype>0 Then wantinput = -1
	...
	ElseIf wantinput=0 Then
		...
		If i="k" Then
			t = findneareststone (mousex, mousey)
		...
		If i="f" Then
			...
			If togglefullscreen Then
				glutFullscreen ()
				SCRX = glutGet (GLUT_SCREEN_WIDTH)
				SCRY = glutGet (GLUT_SCREEN_HEIGHT)
			Else
				SCRX = WINX
				SCRY = WINY
				glutReshapeWindow (SCRX, SCRY)
			EndIf
		EndIf
...
Sub doMotion Cdecl (mx As Integer, my As Integer)
	mx = mx-LL
	my = (SCRY-1)-my

	mousex = (mx-(SCRX-LL)/2)*zoom + (SCRX-LL)/2 + panx
	mousey = (my-SCRY/2)*zoom + SCRY/2 + pany

	searchnearest (mousex, mousey)		' byref! sets x and y
...
Sub doPassiveMotion Cdecl (mx As Integer, my As Integer)
	mx = mx-LL
	my = (SCRY-1)-my
...
Sub doMouse Cdecl (button As Integer, state As Integer, mx As Integer, my As Integer)
	Dim As Integer x, y
	...
	If button=GLUT_LEFT_BUTTON Then
		If state=GLUT_DOWN Then lbuttoncnt = 1
		If state=GLUT_UP Then lbuttoncnt = 0
	EndIf
	If button=GLUT_RIGHT_BUTTON Then
		If state=GLUT_DOWN Then rbuttoncnt = 1
		If state=GLUT_UP Then rbuttoncnt = 0
	EndIf
...
Sub doReshape Cdecl (w As Integer, h As Integer)
	SCRX = w
	SCRY = h
End Sub
...
Sub main ()
	...
	glutInitDisplayMode (GLUT_RGBA Or GLUT_DOUBLE Or GLUT_DEPTH Or GLUT_MULTISAMPLE)
	glutCreateWindow ("RealGo")
